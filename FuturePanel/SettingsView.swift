...
struct SettingsView: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            List {
                SidebarRow(title: "服务器", systemImage: "server.rack", index: 0, selection: $selection)
                SidebarRow(title: "频道", systemImage: "number", index: 1, selection: $selection)
                SidebarRow(title: "DSL 模板", systemImage: "chevron.left.forwardslash.chevron.right", index: 2, selection: $selection)
                SidebarRow(title: "主题", systemImage: "paintbrush", index: 3, selection: $selection)
                SidebarRow(title: "字体", systemImage: "textformat", index: 4, selection: $selection)
                SidebarRow(title: "行为", systemImage: "gear", index: 5, selection: $selection)
            }
            .frame(minWidth: 180)
            .listStyle(.sidebar)
            ...
            Group {
                if selection == 0 {
                    ...
                    ThemePreviewView(store: store)
                } else if selection == 4 {
                    Form {
                        Section(header: Text("字体").font(.headline)) {
                            TextField("字体族（如 SF Mono/JetBrains Mono）", text: $store.settings.fontFamily)
                    ...
                    Form {
                        Section(header: Text("行为").font(.headline)) {
                            Text("这里可添加更多行为开关（自动启动/采样设置等）")
                        }
                    }
                }
            }
